...
Expected format: `SPEC-XXX <new_status>` where `new_status` is one of: `backlog`, `in_progress`, `done`.
...
- If the target status is not one of `backlog`, `in_progress`, `done`, report an error.
